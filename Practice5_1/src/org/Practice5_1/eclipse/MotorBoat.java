	...
	//연료양, 속도 체크 후 에러 메세지 프라이빗변수들은 게터와 세터메소드
	    ...
	    public void AmountOfFuel(double newTime)
	    {
	    	if (newTime > 0.0){
	    		...
	    		System.out.println("current Speed: "+getCurSpeed());
	    		System.out.println("Time: "+Time);
	    		System.out.println("***************\n");
	    		fuelBurned = getEfficiency() * getCurSpeed() * getCurSpeed() * Time;//현재속도
	    		...
	    		fuelBurned = getEfficiency() * maxSpeed * maxSpeed * Time;//최대속도
	    ...
	    public void Distance_sf(){
	    	...
	    	System.out.println("current Speed: "+getCurSpeed());
	    	System.out.println("***************\n");
	    	distance=getFuelAmount()/(getEfficiency()*getCurSpeed());
	    	System.out.println("the travel distance: "+distance);
	    }
	    
	    //equal메소드
	    public boolean equals(MotorBoat motorboat) {
	    	return (motorboat.fuelAmount==this.fuelAmount)&&(motorboat.curSpeed==this.curSpeed)&&(motorboat.efficiency==this.efficiency);
